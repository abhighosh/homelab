...
The artwork selector uses The Met's current paginated `/v1.1/search` API and validates every chosen object as public domain before downloading it. It favours landscapes from the Drawings and Prints collection, with a smaller selection of Asian woodblock prints, because broad compositions and strong shapes survive the panel much better than arbitrary colour paintings or dense engravings. Candidates are assessed at approximately physical display resolution: portrait/extreme aspect ratios, dark works, weak tonal separation and excessive edge density are rejected. Accepted landscape works receive a modest centre crop into a 780×390 image area, eliminating large letterbox margins while protecting the composition. Selection is deterministic for the local date. Both the original JPEG and metadata are cached under Git-ignored `output/daily-artwork/`; an API or network failure retains the latest successful work while weather and astronomy screens continue normally. The image is contrast-adjusted, dithered into the panel's four physical tones and rendered with a narrow, uncluttered caption. `/health` exposes the selected object metadata and display-quality score for diagnostics.
