...
4. **Sky almanac** — three matching sun, moon and today's-weather columns, with sunrise/sunset, moon phase, forecast icon and daily temperatures rendered from data.
...
The five selected night variants use a high-key moonlit treatment designed for the physical e-paper panel: pale stonework and foreground detail against a dark sky, silhouetted branches and weather-specific night cues. This avoids the nearly all-black result produced by the original cloudy-night artwork after four-level quantisation. The original v1 night images remain alongside the selected v2 files for comparison and rollback.
