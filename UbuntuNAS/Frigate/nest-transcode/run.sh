...
readonly QSV_RENDER_DEVICE=/dev/dri/renderD128
...
while true; do
  ...
  # immediately uploaded to the N150 iGPU; QSV retains the encode stage.
  nice -n 10 "${FFMPEG}" \
    -hide_banner \
    -loglevel warning \
    -init_hw_device "qsv=igpu:${QSV_RENDER_DEVICE}" \
